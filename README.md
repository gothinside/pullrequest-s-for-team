...
Чтобы запустить тесты для начала надо поднять тестовое бд и сгенерировать mock'и(make mockgen), поднять тестовое бд и запустить тесты можно через make run_main_test и make e2e-test для запуска e2e тестов(изменено 17.11.2025). Т.К бд тестовое .env file не прилагается. Данные можно заполнить вручную.
...
17.11.2025
Добавил докеры для запуска e2e тестов, изменил makefile для удобства запусков юнит и е2е тестов(юнит тесты включает и интеграционные тесты), изменил структуру папки с тестами для удобства
