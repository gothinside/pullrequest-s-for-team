...
ЕСЛИ У ВАС СТАРАЯ ВЕРСИЯ docker или выхотите запуска именно через docker-compose в MAKEFILE есть запуск и для старых версий(постфикс old) через docker-compose(на новых версиях он может не работать).
