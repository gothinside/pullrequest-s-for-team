...
18.11.2025
Добавил нагрузочное тестирование. Результаты можете посмотреть в папке loat_test/res.txt, 4 ошибки там из-за коллизий(иногда генерируются одинаковые айдишники из-за этого ошибка 400 Already exist, а код ошибки проверяется на 201 и 200, но эти ошибки не в счет по итогу 100%). Также оптимизровал некоторые методы, потоу что до этого сбор статистики очень ухудшал показатели, хотя и без это было <100ms. make с запуском не прилагаю, потому что я запускал из докера. Консинстентность данных соблюдена.
